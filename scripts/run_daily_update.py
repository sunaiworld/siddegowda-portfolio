...
def safe_print(text):
    try:
        ...
        print(text.replace("OK_CHECK", "[OK]").replace("FAIL_CHECK", "[FAIL]"))
...
def main():
    ...
    import_proc = subprocess.run([sys.executable, importer_path], capture_output=True, text=True)
    ...
    engine_proc = subprocess.run([sys.executable, engine_path], capture_output=True, text=True)
